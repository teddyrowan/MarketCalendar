...
#define DATES_INCLUDED 2555  // calendar datediff between first and last day. (2025-12-31 subtract 2019-01-02)
...
+ (int) findSortedIndexInArray:(NSArray*)array withGuess:(int)approx andWithKey:(NSString*)dateString{
    ...
    } while (new_vector != 0 && new_vector == vector);
...
+ (int) dateVectorWithDate:(NSString*)date andComparison:(NSString*)reference{
    ...
    int year_compare = [[date substringToIndex:4] intValue] - [[reference substringToIndex:4] intValue];
    if (year_compare > 0){
        return 1;
    } else if (year_compare < 0){
        return -1;
    } else {
        int month_compare = [[date substringWithRange:NSMakeRange(5, 2)] intValue] - [[reference substringWithRange:NSMakeRange(5, 2)] intValue];
        if (month_compare > 0){
            return 1;
        } else if (month_compare < 0){
            return -1;
        } else {
            int day_compare = [[date substringFromIndex:8] intValue] - [[reference substringFromIndex:8] intValue];
            if (day_compare > 0){
                return 1;
            } else {
                return -1;
